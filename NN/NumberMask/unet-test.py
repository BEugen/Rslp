...
IMG_TEST = '/mnt/misk/misk/lplate/lp-un-mask/img'
PR_RSLT = '/mnt/misk/misk/lplate/lp-un-mask/prd'
fn = []
IMG_SIZE = (64, 256)
...
def testGenerator(test_path, target_size=IMG_SIZE):
    file_list = os.listdir(test_path)
    for i in range(len(file_list)):
        img = cv2.imread(os.path.join(test_path, os.path.splitext(file_list[i])[0] + '.jpg'), cv2.IMREAD_GRAYSCALE)
        if img is None:
            continue
        img = img / 255
...
def main(args):
    ...
    with open("model-un-nb-nn.json", "w") as json_file:
